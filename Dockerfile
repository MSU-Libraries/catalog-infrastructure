...
# Perform updates
RUN apt update && \
    apt install software-properties-common curl gnupg openssh-client git python3-netaddr python3-dnspython -y --no-install-recommends
...
RUN add-apt-repository --yes --update ppa:ansible/ansible && \
    # Install Ansible
    apt install moreutils gettext-base ansible pip openssl -y --no-install-recommends && \
    ...
    apt install terraform -y --no-install-recommends && \
